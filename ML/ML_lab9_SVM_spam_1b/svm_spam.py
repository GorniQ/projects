...
print('Number of non-zero entries: {}\n'.format(np.count_nonzero(features[0])))
...
for i in range(1, len(weights[0])+1):
    print(vocabulary_dict[i])
    list_of_wrds.append((weights[0][i-1], vocabulary_dict[i]))

list_of_wrds = sorted(list_of_wrds, key=lambda x: x[0], reverse=True)
...
for i in range(15):
    ...
    print(' {word:<20}: {weight:10.6f}'.format(
        word=list_of_wrds[i][1], weight=list_of_wrds[i][0]))
