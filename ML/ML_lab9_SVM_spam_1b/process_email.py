...
def process_email(email_contents: str) -> List[int]:
    ...
    for token in tokens:
        ...
        if token in vocabulary_dict.values():
            idx = list(vocabulary_dict.keys())[
                list(vocabulary_dict.values()).index(token)]
